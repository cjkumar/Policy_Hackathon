...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 16 21:40:38 2024
...
# Function to generate normalized heatmap for each combination of age, ethnicity, and gender
def generate_normalized_heatmap(data, age_group, ethnicity, gender, output_dir):
    ...
    filename = f"{ethnicity.replace(' ', '_')}_{age_group.replace(' ', '_')}_{gender}.png"
    filepath = os.path.join(output_dir, filename)
    plt.savefig(filepath)
    plt.close()

# Generate and save heatmaps for each combination of age group, ethnicity, and gender
age_groups = ["Under 65", "65 and Over"]
genders = ["Male", "Female"]
for age_group in age_groups:
    for ethnicity in relevant_ethnicities:
        for gender in genders:
            generate_normalized_heatmap(data, age_group, ethnicity, gender, output_dir)